...
import { FiLogOut, FiUser, FiBell, FiInbox } from "react-icons/fi";
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        // Get only pending requests as notifications
        const pending = data.filter(r => r.status === 'pending').slice(0, 5);
        setNotifications(pending);
      } catch (err) {
        console.error("Failed to fetch owner notifications");
      }
    };
    fetchNotifications();
  }, []);
  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl border-b border-gray-100 px-8 py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
      <div className="flex items-center gap-2">
        <h1 className="text-xl font-bold text-gray-800 tracking-tight hidden md:block">Owner Dashboard</h1>
      </div>
      ...
      <div className="flex items-center gap-6">
              ...
              <div className="max-h-80 overflow-y-auto">
                {notifications.length === 0 ? (
                  <div className="text-center py-6 flex flex-col items-center">
                    ...
                    <p className="text-sm text-gray-500">No new booking requests</p>
                  </div>
                ) : (
                  notifications.map(n => (
                    <div key={n._id} className="p-4 border-b border-gray-50 hover:bg-gray-50 transition-colors flex gap-3 cursor-pointer" onClick={() => navigate('/owner/requests')}>
                      <div className="mt-1 flex-shrink-0 w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center">
                        <FiUser size={14} />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-gray-800">{n.farmer?.name} wants to book</p>
        ...
        <Link to="/owner/profile" className="flex items-center gap-3 cursor-pointer group">
          <div className="hidden md:flex flex-col text-right">
            <span className="text-sm font-bold text-gray-800 leading-tight group-hover:text-green-600 transition-colors">{user?.name || 'Equipment Owner'}</span>
